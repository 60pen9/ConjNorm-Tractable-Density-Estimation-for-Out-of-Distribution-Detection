...
total_features = np.zeros((1, 1280))
...
np.save("./imagenet_feature_mobilenet_v2/imagnet_labels.npy", total_labels)
...
for j in range(1000):
    ...
    np.save("./imagenet_feature_mobilenet_v2/class_{j}.npy".format(j=j), total_features_j[1:])

val1 = np.load("val_features_0097.npz")
features1 = val1["features"]
np.save("./imagenet_feature_mobilenet_v2/val.npy", features1)
