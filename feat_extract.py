...
for split, in_loader in [('train', trainloaderIn), ('val', testloaderIn),]:
    ...
    cache_name_feat = f"cache/{args.in_dataset}_{split}_{args.name}_in_alllayers_feat.npy"
    cache_name_label = f"cache/{args.in_dataset}_{split}_{args.name}_in_alllayers_label.npy"
    if FORCE_RUN or not os.path.exists(cache_name_feat):
...
for ood_dataset in args.out_datasets:
    ...
    if FORCE_RUN or not os.path.exists(cache_name_feat):
        ...
        np.save(cache_name_feat, ood_feat_log.T)
    else:
        ood_feat_log = np.load(cache_name_feat, allow_pickle=True)
