...
class_num = 1000
prepos_feat = lambda x: np.ascontiguousarray(x).astype(np.float64)
t1 = 0.005
ord_def = 1.8
q = 1/(1-1/ord_def)
...
feat_log_val = np.load("./imagenet_mobilenet/imagenet_feature_mobilenet_v2/val.npy")# you should load from your own address
...
for k in range(class_num):
    k_ftrain = np.load("./imagenet_mobilenet/imagenet_feature_mobilenet_v2/class_{k}.npy".format(k=k)) # you should load from your own address
    class_protytypes[k, :] = prepos_feat(np.mean(k_ftrain, axis=0))
